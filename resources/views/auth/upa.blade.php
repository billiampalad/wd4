...
<html lang="id" data-theme="light"
    class="{{ request()->routeIs('*.analitik.status-kerjasama') ? 'status-kerjasama-page' : '' }} {{ request()->routeIs('*.analitik.klasifikasi-mitra') ? 'klasifikasi-mitra-page' : '' }} {{ request()->routeIs('*.analitik.geo-mitra') ? 'geo-mitra-page' : '' }} {{ request()->routeIs('*.institusi') ? 'institusi-page' : '' }} {{ request()->routeIs('*.evaluasi') ? 'evaluasi-kinerja-page' : '' }} {{ request()->routeIs('*.referensi.bentuk-kegiatan') ? 'bentuk-kegiatan-page' : '' }} {{ request()->routeIs('*.laporan') ? 'laporan-page' : '' }}">
...
<body
    ...
    @php
        ...
        if ($notificationUser) {
            $notificationUser->loadMissing('profile.upa');
            ...
            if ($notificationProfile?->upa_id) {
                $expiryQuery->where(function ($query) use ($notificationProfile) {
                    $query->where('upa_id', $notificationProfile->upa_id)
                        ->orWhereHas('upas', fn($subQuery) => $subQuery->where('upas.id', $notificationProfile->upa_id));
                });
            } else {
    ...
    <div class="layout">
        ...
        <aside id="sidebar">
            ...
            <a class="menu-item {{ request()->routeIs('upa.laporan', 'upa.laporan.*') ? 'active' : '' }}"
                href="{{ route('upa.laporan') }}">
