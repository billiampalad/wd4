<main id="mainContent">
    <div class="page-header">
        <div class="breadcrumb">
            ...
            <span class="sep">/</span>
            <span class="current">Laporan Global</span>
        </div>
        <h2>Laporan Data Global</h2>
        <p>Saring dan unduh dokumen laporan kerjasama secara kolektif dari seluruh Jurusan dan Unit Kerja.</p>
    </div>
    ...
    <div class="report-filter-container" x-data="{ showFilters: true }">
        ...
        <div class="rfc-body" x-show="showFilters" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 transform -translate-y-4" x-transition:enter-end="opacity-100 transform translate-y-0" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100 transform translate-y-0" x-transition:leave-end="opacity-0 transform -translate-y-4">
            <form id="filterForm" class="rfc-form"
                  data-preview-url="{{ route('pimpinan.laporan.preview') }}"
                  data-pdf-url="{{ route('pimpinan.laporan.pdf') }}"
                  data-excel-url="{{ route('pimpinan.laporan.excel') }}">

                <div class="rfc-grid" style="grid-template-columns: repeat(auto-fit, minmax(200px, 1fr));">
                    <!-- Column 1: Tanggal Awal -->
                    ...
                    <div class="rfc-group" x-data="{
                        ...
                        <input type="hidden" name="kategori_mitra" :value="selected">
                        <div class="alpine-dropdown" @click.outside="open = false">
                            <div class="ad-trigger" :class="{'active': open}" @click="open = !open">
                                <div style="display: flex; align-items: center; gap: 12px;">
                    ...
                    <!-- Column 5: Pengusul / Pelaksana -->
                    <div class="rfc-group" x-data="{
                        open: false,
                        selected: 'all',
                        selectedLabel: 'Semua Kampus (Global)',
                        items: [
                            { id: 'all', label: 'Semua Kampus (Global)' },
                            { id: 'optgroup_jurusan', label: '── JURUSAN ──', disabled: true },
                            @foreach($jurusans as $j)
                            { id: 'jurusan_{{ $j->id }}', label: '{{ $j->nama_jurusan }}' },
                            @endforeach
                            { id: 'optgroup_unit', label: '── UNIT KERJA ──', disabled: true },
                            @foreach($units as $u)
                            { id: 'unit_{{ $u->id }}', label: '{{ $u->nama_unit_pelaksana }}' },
                            @endforeach
                        ]
                    }">
                        <label>Pengusul / Pelaksana</label>
                        <input type="hidden" name="pengusul" :value="selected">
                        <div class="alpine-dropdown" @click.outside="open = false">
                            <div class="ad-trigger" :class="{'active': open}" @click="open = !open">
                                <div style="display: flex; align-items: center; gap: 12px;">
                                    <i class="fas fa-university" style="color: #9ca3af; font-size: 13px;"></i>
                                    <span x-text="selectedLabel"></span>
                                </div>
                                <i class="fas fa-chevron-down" style="font-size: 10px; transition: 0.3s" :style="open ? 'transform: rotate(180deg)' : ''"></i>
                            </div>
                            <div class="ad-menu" x-show="open" x-transition>
                                <template x-for="item in items">
                                    <div class="ad-item" :class="{'selected': selected == item.id, 'ad-optgroup': item.disabled}"
                                         @click="if(!item.disabled) { selected = item.id; selectedLabel = item.label; open = false }"
                                         x-text="item.label"></div>
                                </template>
                            </div>
                        </div>
    ...
    <div class="card um-card">
        <div class="card-header um-header" style="display: flex; justify-content: space-between; align-items: center; padding: 15px 20px;">
            <div class="card-title" style="font-weight: 700; color: var(--text); display: flex; align-items: center; gap: 10px;">
                ...
                <span>Preview Ringkasan Laporan</span>
                ...
                <table id="previewTable" class="um-table" style="white-space: nowrap; min-width: 1000px;">
                    <thead>
                        <tr>
                            ...
                            <th class="um-th">Pengusul (Jurusan/Unit)</th>
                            <th class="um-th">Nama Kegiatan</th>
                            <th class="um-th">Mitra & Kategori</th>
                            <th class="um-th">Jenis Kerjasama</th>
                            <th class="um-th">Periode Pelaksanaan</th>
...
document.addEventListener('DOMContentLoaded', function () {
        ...
        fetch(url, { headers: { 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest' } })
            ...
            .then(function (data) {
                ...
                if (!data || data.length === 0) {
                    previewBody.innerHTML = '<tr><td colspan="7" class="um-empty"><div class="um-empty-state" style="padding:30px 0;"><div class="um-empty-icon"><i class="fas fa-inbox" style="font-size:28px; opacity:0.3; color:var(--text-sub);"></i></div><p class="um-empty-title">Tidak ada data ditemukan</p><p class="um-empty-sub">Coba ubah filter untuk menampilkan data lain.</p></div></td></tr>';
                    ...
                    data.forEach(function (item, index) {
                        ...
                        tr.className = 'um-tr';
                        tr.innerHTML = `
                            <td class="um-td um-td-num">${index + 1}</td>
                            <td class="um-td"><strong>${item.pengusul_label}</strong></td>
                            <td class="um-td">${item.nama_kegiatan}</td>
                            <td class="um-td">${item.mitra_info}</td>
                            <td class="um-td">${item.jenis_kerjasama_label}</td>
                            <td class="um-td">${item.periode_label}</td>
                            <td class="um-td"><span class="tag ${item.status_class}">${item.status_label}</span></td>
                        `;
            ...
            .catch(function (err) {
                console.error(err);
                previewBody.innerHTML = '<tr><td colspan="7" class="um-empty"><p style="color:var(--tag-red-text)">Gagal memuat data.</p></td></tr>';
            })
            .finally(function () {
                btn.disabled = false;
    ...
    document.getElementById('btnExportExcel').addEventListener('click', function () {
        window.location.href = form.dataset.excelUrl + '?' + getFormParams();
    });

    // Auto-trigger on page load to show initial data
    setTimeout(() => {
        document.getElementById('btnTampilkan').click();
    }, 100);
});
</script>