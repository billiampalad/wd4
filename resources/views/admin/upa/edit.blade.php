@extends('admin.dashboard')

@section('content')
<main class="main-content">
    <div class="page-header">
        <div class="breadcrumb">
            <i class="fas fa-building-columns"></i>
            <span class="sep">/</span>
            <a href="{{ route('upa.index') }}" style="color: inherit; text-decoration: none;">UPA</a>
            <span class="sep">/</span>
            <span class="current">Edit UPA</span>
        </div>
        <h2 id="pageTitle">Edit UPA</h2>
        <p id="pageDesc">Ubah data UPA yang sudah ada.</p>
    </div>
    
    <div class="card ue-form-card" style="max-width: 600px; margin: 0 auto;">
        <div class="card-header ue-form-header">
            <div class="card-title"><i class="fas fa-edit"></i> Formulir Edit UPA</div>
            <span class="ue-edit-badge">
                <i class="fas fa-circle" style="font-size:7px;"></i> Mode Edit
            </span>
        </div>

        <form action="{{ route('upa.update', $upa->id) }}" method="POST">
            @csrf
            @method('PUT')

            <div class="card-body ue-body">
                <div class="ue-section-label">
                    <span class="ue-section-num">01</span>
                    <span>Informasi UPA</span> 
                </div>

                <div class="ue-form-group">
                    <label class="ue-label" for="nama_upa">
                        <i class="fas fa-building-columns ue-label-icon"></i>
                        Nama UPA
                        <span class="ue-required">*</span>
                    </label>
                    <input
                        type="text" id="nama_upa" name="nama_upa"
                        class="ue-input @error('nama_upa') ue-input-error @enderror"    
                        value="{{ old('nama_upa', $upa->nama_upa) }}"
                        placeholder="Ubah nama UPA"
                        required
                        maxlength="150"
                    >
                    @error('nama_upa')   
                        <span class="ue-error-msg"><i class="fas fa-circle-exclamation"></i> {{ $message }}</span>
                    @enderror
                </div>
            </div>

            <div class="card-footer ue-footer">
                <a href="{{ route('upa.index') }}" class="ue-btn-cancel">
                    <i class="fas fa-arrow-left"></i> Batal
                </a>
                <button type="submit" class="ue-btn-submit">
                    <i class="fas fa-floppy-disk"></i> Simpan Perubahan
                </button>
            </div>
        </form>
    </div>
</main>
@endsection
