                    ...
                    <tr class="um-row">
                        ...
                        <td class="um-td um-td-aksi">
                            <div class="actions um-actions">
                                ...
                                <form id="form-delete-klasifikasi-{{ $item->id }}" action="{{ route('klasifikasi.destroy', $item->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" class="btn-action delete um-btn-delete" title="Hapus" onclick="CustomAlert.confirm({
                                        title: 'Hapus Klasifikasi',
                                        message: 'Menghapus klasifikasi <span class=\'custom-alert-highlight\'>{{ addslashes($item->nama) }}</span> tidak dapat dikembalikan.',
                                        type: 'danger',
                                        confirmText: 'Hapus',
                                        confirmColor: 'danger',
                                        onConfirm: () => document.getElementById('form-delete-klasifikasi-{{ $item->id }}').submit()
                                    })">
