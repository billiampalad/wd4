...
// --- LOGIKA FORM MODAL PENGANJUAN PERPANJANGAN (HUMAS / UNIT KERJA) ---
function initPerpanjanganModalHandlers() {
    const modal = document.getElementById('modalProsesPerpanjangan');
    if (!modal) return;

    document.querySelectorAll('.btn-proses-perpanjangan').forEach(btn => {
        btn.addEventListener('click', function () {
            const id = this.getAttribute('data-id');
            const mitra = this.getAttribute('data-mitra');
            const kode = this.getAttribute('data-kode');
            const judul = this.getAttribute('data-judul');
            const docnumber = this.getAttribute('data-docnumber');
            ...
            const filesurat = this.getAttribute('data-filesurat');

            const form = document.getElementById('formProsesPerpanjangan');
            if (form) {
                form.action = `/unit/pengajuan-perpanjangan/${id}/proses`;
            }

            const subTitleEl = document.getElementById('modalSubtitleMitra');
            if (subTitleEl) subTitleEl.textContent = `Lengkapi nomor dokumen & berkas untuk ${mitra || 'Mitra'}`;

            const kodeEl = document.getElementById('modalKodeSubmission');
            if (kodeEl) kodeEl.textContent = kode || 'KODE';

            const judulEl = document.getElementById('modalJudulPengajuan');
            if (judulEl) judulEl.textContent = judul || '-';

            const tujuanEl = document.getElementById('modalTujuanPengajuan');
            if (tujuanEl) tujuanEl.textContent = tujuan || '-';
            ...
            const suratLink = document.getElementById('modalFileSuratLink');
            if (suratLink) {
                if (filesurat) {
                    suratLink.href = filesurat;
                    suratLink.style.display = 'inline-flex';
                } else {
...
function submitPerpanjanganForm(statusAksi) {
    const inputStatus = document.getElementById('inputStatusAksi');
    if (inputStatus) {
        inputStatus.value = statusAksi;
    }
    ...
    const titleMsg = statusAksi === 'aktif' 
        ? 'Sahkan & Aktifkan Perpanjangan?' 
        : 'Simpan Draf Perpanjangan?';
    const textMsg = statusAksi === 'aktif'
        ? 'Status kerja sama baru akan diubah menjadi Aktif dan tercatat di master data.'
        : 'Data perpanjangan akan disimpan sebagai draf sementara.';

    if (typeof Swal !== 'undefined') {
        Swal.fire({
            title: titleMsg,
            text: textMsg,
            icon: statusAksi === 'aktif' ? 'question' : 'info',
            showCancelButton: true,
            confirmButtonColor: statusAksi === 'aktif' ? '#10b981' : '#2563eb',
            cancelButtonColor: '#64748b',
            confirmButtonText: statusAksi === 'aktif' ? 'Ya, Sahkan & Aktifkan!' : 'Ya, Simpan Draf',
            cancelButtonText: 'Batal'
        }).then((result) => {
            if (result.isConfirmed) {
                form.submit();
...
window.closeProsesModal = closeProsesModal;
window.submitPerpanjanganForm = submitPerpanjanganForm;

document.addEventListener('DOMContentLoaded', initPerpanjanganModalHandlers);
document.addEventListener('turbo:load', initPerpanjanganModalHandlers);
